...
function createClient(data){var ctx;try{ctx=requirePermission_(AUTH_PERMISSIONS.CLIENTS);}catch(e){throw new Error('Client creation failed at authorization: '+(e&&e.message?e.message:String(e)));}try{return createClientService_(data);}catch(e){var message=e&&e.message?e.message:String(e);logError_('createClient failed',e);throw new Error('Client creation failed: '+message);}}
...
function getDeliverableAssignees(){requirePermission_(AUTH_PERMISSIONS.DELIVERABLES);var s=getCRMSpreadsheet_().getSheetByName(APP.SHEETS.USERS);if(!s||s.getLastRow()<2)return[];var v=s.getDataRange().getValues(),h=v.shift(),nameIdx=h.indexOf('Name'),roleIdx=h.indexOf('Role'),jobIdx=h.indexOf('Job Function'),emailIdx=h.indexOf('Email'),statusIdx=h.indexOf('Status');return v.filter(function(r){return String(r[statusIdx]||'Active')==='Active';}).map(function(r){return{userId:String(r[0]||''),name:String(r[nameIdx]||''),role:String(r[roleIdx]||''),jobFunction:jobIdx>=0?String(r[jobIdx]||''):'',email:String(r[emailIdx]||'')};});}
function createContent(data){requirePermission_(AUTH_PERMISSIONS.CONTENT);return createContentService_(data);} function getContent(deliverableId){requirePermission_(AUTH_PERMISSIONS.CONTENT);return getContentService_(deliverableId);} function getContentAssignees(){requirePermission_(AUTH_PERMISSIONS.CONTENT);return getContentAssigneesService_();} function updateContent(id,data){requirePermission_(AUTH_PERMISSIONS.CONTENT);return updateContentService_(id,data);}
function getContentForDeliverable(deliverableId){requirePermission_(AUTH_PERMISSIONS.CONTENT);return getContentForDeliverableService_(deliverableId);} function linkContentTask(contentId,taskId){requirePermission_(AUTH_PERMISSIONS.TASKS);return linkContentTaskService_(contentId,taskId);} function unlinkContentTask(contentId,taskId){requirePermission_(AUTH_PERMISSIONS.TASKS);return unlinkContentTaskService_(contentId,taskId);} function getContentTaskBundle(contentId){requirePermission_(AUTH_PERMISSIONS.CONTENT);return getContentTaskBundleService_(contentId);} function getContentTaskLinks(contentId,taskId){requirePermission_(AUTH_PERMISSIONS.CONTENT);return getContentTaskLinksService_(contentId,taskId);}
...
function createTask(data){var c=requirePermission_(AUTH_PERMISSIONS.TASKS);if(!canManageTasks_(c))throw new Error('Only Admin, Partner or Employee SMM can create tasks.');return createTaskService_(data);}
