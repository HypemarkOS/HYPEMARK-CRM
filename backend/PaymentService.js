...
function ensurePaymentHeaders_(){
  ...
  var headersNow=s.getRange(1,1,1,s.getLastColumn()).getValues()[0];
  var typeIndex=headersNow.indexOf('Type'); var clientIndex=headersNow.indexOf('Client ID');
  if(typeIndex>=0&&clientIndex>=0&&s.getLastRow()>1){
    var rows=s.getRange(2,1,s.getLastRow()-1,s.getLastColumn()).getValues();
    var changed=false;
    rows.forEach(function(r){if(r[typeIndex]===''&&r[clientIndex]!==''){r[typeIndex]=PAYMENT_TYPES.CLIENT_RECEIPT;changed=true;}});
    if(changed)s.getRange(2,1,rows.length,s.getLastColumn()).setValues(rows);
  }
...
function createPaymentService_(data){
  ...
  var item={'Payment ID':generateId_(CONFIG.ID_PREFIX.PAYMENT),'Type':type,'Client ID':clientId,'Amount':amount,'Payment Date':data.paymentDate?new Date(data.paymentDate):now_(),'Paid To':asText_(data.paidTo),'Employee':asText_(data.employee),'Expense Category':category,'Related To':relatedTo,'Description':asText_(data.description),'Status':asText_(data.status)||(type===PAYMENT_TYPES.CLIENT_RECEIPT?'Received':'Completed')};
...
function getClientProfitabilityService_(clientId){var payments=getPaymentsService_(clientId),receipts=0,employee=0,misc=0;payments.forEach(function(p){var a=Number(p['Amount']||0);if(p['Type']===PAYMENT_TYPES.CLIENT_RECEIPT)receipts+=a;if(p['Type']===PAYMENT_TYPES.EMPLOYEE_PAYMENT)employee+=a;if(p['Type']===PAYMENT_TYPES.MISC_EXPENSE)misc+=a;});return {clientId:clientId,receipts:receipts,employeePayments:employee,miscExpenses:misc,totalCosts:employee+misc,profit:receipts-employee-misc};}
