function initializeCRMService_() {
  ...
  migrateLegacyPayments_(ss); applyFinanceValidations_(ss); applyReceiptBalanceFormulas_(ss); ensureCRMTriggers_(ss);
  var defaultSheet=ss.getSheetByName('Sheet1'); if(defaultSheet&&ss.getSheets().length>1&&defaultSheet.getLastRow()===0&&defaultSheet.getLastColumn()===1)ss.deleteSheet(defaultSheet);
  console.log('HYPEMARK CRM spreadsheet: '+ss.getUrl()); return ss.getUrl();
}
function ensureHeaders_(sheet,headers){var current=sheet.getLastColumn()||0;if(sheet.getLastRow()===0){sheet.getRange(1,1,1,headers.length).setValues([headers]);return;}var existing=sheet.getRange(1,1,1,current).getValues()[0];headers.forEach(function(h){if(existing.indexOf(h)===-1)sheet.getRange(1,sheet.getLastColumn()+1).setValue(h);});}
...
function applyFinanceValidations_(ss){
  ...
  var employees=ss.getSheetByName(APP.SHEETS.EMPLOYEE_PAYMENTS); if(employees){employees.getRange('B2:B').setDataValidation(employeeRule);if(clientRule)employees.getRange('C2:C').setDataValidation(clientRule);if(receiptRule)employees.getRange('D2:D').setDataValidation(receiptRule);employees.getRange('E2:E').setDataValidation(employeeTypeRule);employees.getRange('K2:K').setDataValidation(accountRule);employees.getRange('L2:L').setDataValidation(modeRule);}
  var expenses=ss.getSheetByName(APP.SHEETS.BUSINESS_EXPENSES); if(expenses){if(clientRule)expenses.getRange('B2:B').setDataValidation(clientRule);if(receiptRule)expenses.getRange('C2:C').setDataValidation(receiptRule);expenses.getRange('D2:D').setDataValidation(categoryRule);expenses.getRange('E2:E').setDataValidation(contextRule);expenses.getRange('H2:H').setDataValidation(employeeRule);expenses.getRange('J2:J').setDataValidation(modeRule);}
}
function applyReceiptBalanceFormulas_(ss){var receipts=ss.getSheetByName(APP.SHEETS.CLIENT_RECEIPTS);if(!receipts)return;var lastRow=Math.max(receipts.getLastRow(),2);receipts.getRange('J1:K1').setValues([['Allocated Amount','Remaining Balance']]);receipts.getRange('J2').setFormula('=IF(A2="","",SUMIF(EmployeePayments!D:D,A2,EmployeePayments!F:F)+SUMIF(BusinessExpenses!C:C,A2,BusinessExpenses!F:F))');receipts.getRange('K2').setFormula('=IF(A2="","",C2-J2)');if(lastRow>2)receipts.getRange('J2:K2').copyTo(receipts.getRange(2,10,lastRow-1,2));receipts.getRange('C2:C').setNumberFormat('₹#,##0.00');receipts.getRange('J2:K').setNumberFormat('₹#,##0.00');}
function migrateLegacyPayments_(ss){var legacy=ss.getSheetByName(APP.SHEETS.PAYMENTS);if(!legacy||legacy.getLastRow()<2){if(legacy&&ss.getSheets().length>1)ss.deleteSheet(legacy);return;}var values=legacy.getDataRange().getValues(),headers=values.shift(),index=function(name){return headers.indexOf(name);};var receiptSheet=ss.getSheetByName(APP.SHEETS.CLIENT_RECEIPTS),employeeSheet=ss.getSheetByName(APP.SHEETS.EMPLOYEE_PAYMENTS),expenseSheet=ss.getSheetByName(APP.SHEETS.BUSINESS_EXPENSES);values.forEach(function(r){if(!r.some(function(x){return x!=='';}))return;var type=index('Type')>=0?String(r[index('Type')]||''):'Client Receipt',id=index('Payment ID')>=0?r[index('Payment ID')]:'',client=index('Client ID')>=0?r[index('Client ID')]:'',amount=index('Amount')>=0?r[index('Amount')]:'' ,date=index('Payment Date')>=0?r[index('Payment Date')]:'',paidTo=index('Paid To')>=0?r[index('Paid To')]:'',employee=index('Employee')>=0?r[index('Employee')]:'',category=index('Expense Category')>=0?r[index('Expense Category')]:'',related=index('Related To')>=0?r[index('Related To')]:'',description=index('Description')>=0?r[index('Description')]:'',status=index('Status')>=0?r[index('Status')]:'Completed';if(type==='Employee Payment')employeeSheet.appendRow([id||generateId_(CONFIG.ID_PREFIX.EMPLOYEE_PAYMENT),employee||'Ajay',client,'','Other',amount,date,'','','',paidTo,'','','',status,description]);else if(type==='Miscellaneous Expense'||type==='Business Expense')expenseSheet.appendRow([id||generateId_(CONFIG.ID_PREFIX.BUSINESS_EXPENSE),client,'',category||'Other',related||'Other',amount,date,'',paidTo,'','','',status]);else receiptSheet.appendRow([id||generateId_(CONFIG.ID_PREFIX.CLIENT_RECEIPT),client,amount,date,paidTo,'','',status||'Received',description,'','']);});ss.deleteSheet(legacy);}
